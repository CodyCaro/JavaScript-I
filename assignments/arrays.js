...
for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].id == 33) {
        console.log("Car 33 is a " + inventory[i].car_make + " " + inventory[i].car_model + ".");
    }
}
...
//     carModels.sort();
// }
const carModels = inventory.map((inv, index) => {
...
//     carYears.sort();
// }
const carYears = inventory.map((inv, index) => {
    return inv.car_year;
});
...
//     }
// }
const oldCars = carYears.filter((year, index) => {
    if (year < 2000) {
        return year;
    }
});
...
//     }
// }
const BMWAndAudi = inventory.filter((inv, index) => {
